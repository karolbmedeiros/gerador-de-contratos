...
from gerar_contrato import gerar_docx, gerar_termo_quitacao, gerar_notificacao_avalista, gerar_notificacao_inadimplente, gerar_ordem_servico, gerar_vistoria_entrega, gerar_vistoria_nova, nome_arquivo_saida
...
def pagina_ordem_servico():
    return render_template("ordem_servico.html", active="ordem_servico")


@app.route("/ordem-servico", methods=["POST"])
def processar_ordem_servico():
    dados = {
        "cliente":           request.form.get("cliente", ""),
        "preenchido_por":    request.form.get("preenchido_por", ""),
        "endereco":          request.form.get("endereco", ""),
        ...
        "sintomas":          request.form.get("sintomas", ""),
        "assinatura_cliente": request.form.get("assinatura_cliente", ""),
        "assinatura_func":    request.form.get("assinatura_func", ""),
        # Acessórios
        "calotas":          request.form.get("calotas", ""),
        "buzina":           request.form.get("buzina", ""),
        "doc_crlv":         request.form.get("doc_crlv", ""),
        "triangulo":        request.form.get("triangulo", ""),
        "antena":           request.form.get("antena", ""),
        "sensor_re":        request.form.get("sensor_re", ""),
        "som_alto_falante": request.form.get("som_alto_falante", ""),
        "tapetes":          request.form.get("tapetes", ""),
        "limpadores":       request.form.get("limpadores", ""),
        "chave_roda":       request.form.get("chave_roda", ""),
        "vidros_elet":      request.form.get("vidros_elet", ""),
        "oleo_motor":       request.form.get("oleo_motor", ""),
        "alarme_trava":     request.form.get("alarme_trava", ""),
        "lampadas":         request.form.get("lampadas", ""),
        "macaco_mec":       request.form.get("macaco_mec", ""),
        "estepe":           request.form.get("estepe", ""),
        "func_gnv":         request.form.get("func_gnv", ""),
        "agua":             request.form.get("agua", ""),
        "borracha_psg_d":   request.form.get("borracha_psg_d", ""),
        "borr_mtr":         request.form.get("borr_mtr", ""),
        "asa_urubu_dd":     request.form.get("asa_urubu_dd", ""),
        "asa_urub_td":      request.form.get("asa_urub_td", ""),
        "tapete_mala":      request.form.get("tapete_mala", ""),
        "tampa_prx":        request.form.get("tampa_prx", ""),
        "borracha_psg_t":   request.form.get("borracha_psg_t", ""),
        "borr_mtr_t":       request.form.get("borr_mtr_t", ""),
        "asa_urubu_de":     request.form.get("asa_urubu_de", ""),
        "asa_urub_te":      request.form.get("asa_urub_te", ""),
        "bagagito":         request.form.get("bagagito", ""),
        "linguet":          request.form.get("linguet", ""),
    }

    formato = request.form.get("formato", "pdf").lower()
    placa     = _slugify(dados.get("placa", "PLACA"))
    data_slug = datetime.now().strftime("%d.%m.%Y")
    nome_docx = f"OS_{placa}_{data_slug}.docx"
    nome_pdf  = f"OS_{placa}_{data_slug}.pdf"
    caminho_docx = str(CONTRATOS_FOLDER / nome_docx)
    caminho_pdf  = str(CONTRATOS_FOLDER / nome_pdf)

    try:
        gerar_ordem_servico(dados, caminho_docx)
    except Exception as e:
        import traceback; traceback.print_exc()
        return jsonify({"error": f"Erro ao gerar OS: {e}"}), 500

    def _salvar_historico(arquivo):
        h = get_historico()
        h.append({
            "id": uuid.uuid4().hex,
            "locatario_nome": dados.get("cliente", ""),
    ...
    if formato == "docx":
        _salvar_historico(nome_docx)
        return jsonify({"download_url": url_for("baixar_ordem_servico", nome=nome_docx)})

    try:
        _converter_pdf(caminho_docx, caminho_pdf)
    except Exception as e:
        return jsonify({"error": f"Erro ao converter PDF: {e}"}), 422

    _salvar_historico(nome_pdf)
    return jsonify({"download_url": url_for("baixar_ordem_servico", nome=nome_pdf)})


@app.route("/ordem-servico/download/<nome>")
def baixar_ordem_servico(nome):
    caminho = CONTRATOS_FOLDER / nome
    if not caminho.exists() or caminho.parent.resolve() != CONTRATOS_FOLDER.resolve():
        abort(404)
    ext = caminho.suffix.lower()
