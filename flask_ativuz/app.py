...
@app.route("/historico/download/<filename>")
def download_contrato(filename):
    safe = secure_filename(filename)
    caminho = CONTRATOS_FOLDER / safe
...
@app.route("/historico/download-pdf/<filename>")
def download_contrato_pdf(filename):
    safe = secure_filename(filename)
    caminho_docx = CONTRATOS_FOLDER / safe
    if not caminho_docx.exists():
        return jsonify({"error": "Arquivo não encontrado."}), 404

    nome_pdf    = safe.replace(".docx", ".pdf")
